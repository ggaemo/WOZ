...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, utterance_type_ids=None):
        ...
        if utterance_type_ids is None:
            ...
            utterance_type_embeddings = torch.cat(  # CLS, D_AGG, D_0, ..., D_29, [SEP]
                [utterance_type_ids.new_zeros((inputs_embeds.shape[0], 1 + 1 + 30 + 1, inputs_embeds.shape[2]),
                                              dtype=torch.float),
...
class AlbertModel(AlbertPreTrainedModel):
    ...
    Return:
        ...
        pooled_domain_output = self.pooler_activation(self.pooler_sept(sequence_output[domain_token_mask]))

        pooled_domain_output = pooled_domain_output.view(pooled_output.shape[0], len(self.config.domain_cls_tokens), -1)

        outputs = (sequence_output, pooled_output, pooled_domain_output) + encoder_outputs[1:]  # add hidden_states and attentions if they are here
        return outputs
...
class AlbertForDST(AlbertPreTrainedModel):
    def __init__(self, config):
        ...
        print('my modeling albert SOP 26')
        super().__init__(config)
        ...
        self.domain_gate_classifier_list = \
            nn.ModuleList([nn.Linear(config.hidden_size * 2, self.num_gate_label) for _ in range(self.num_domain_slot)])
        # self.domain_gate_classifier = nn.Linear(config.hidden_size, self.num_gate_label)
        self.dropout_2 = nn.Dropout(config.classifier_dropout_prob)
        ...
        if config.version == '1':
            ...
            with open('data/WOZ22_slot_value_dict', 'rb') as f:
                slot_value_dict = pickle.load(f)
        # print('VERSION 2.2' * 200)
        # with open('data/WOZ22_slot_value_dict', 'rb') as f:
        #     slot_value_dict = pickle.load(f)

        print('USING PREMADE SLOT VALUE DICT' * 100)


        self.domain_slot_value_classifier_list = \
            nn.ModuleList([nn.Linear(config.hidden_size * 2 , len(slot_value_list)) for slot_value_list in slot_value_dict.values()])

        self.dropout_3 = nn.Dropout(config.classifier_dropout_prob)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if slot_labels is not None:
            domain_slot_value_prediction_scores = [x(self.dropout_3(
                torch.cat([pooled_output, domain_slot_pooled_output[:, i]], 1))) for i, x in
                enumerate(self.domain_slot_value_classifier_list)]

            # domain_slot_value_prediction_scores = [
            #     x(self.dropout_3(pooled_output)) for i, x in
            #     enumerate(self.domain_slot_value_classifier_list)]
            for idx, pred_score in enumerate(domain_slot_value_prediction_scores):
                if idx == 0:
                    slot_loss = loss_fct(pred_score, slot_labels[:, idx])
                else:
                    slot_loss += loss_fct(pred_score, slot_labels[:, idx])
