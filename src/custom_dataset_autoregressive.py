...
class WoZDSTDataset(Dataset):
 ...
 'taxi-arrive by']
        ...
        if os.path.exists(f'data/{data_option}_{max_turns}_{data_type}.pkl'):
            ...
            # print('SGD DATA ADDED')
            # if data_option == 'train':
            #     with open('data/sgd_dataset.pkl', 'rb') as f:
            #         sgd_data = pickle.load(f)
            #
            #     sgd_prev_slot_label_list = list()
            #     for prev_state_tmp in sgd_data['prev_ds_state']:
            #         slot_label_list_tmp = ['none'] * 30
            #         for k, v in prev_state_tmp.items():
            #             slot_label_list_tmp[self.domain_slot_list.index(k)] = v
            #         sgd_prev_slot_label_list.append(slot_label_list_tmp)
            #
            #     sgd_dialog_history = get_usr_sys_dialog(sgd_data['data_utterance'])
            #     sgd_modified_slots = [np.array(x) for x in sgd_data['modified_slot_list']]
            #
            #     self.dialog_history.extend(sgd_dialog_history)
            #     self.prev_slot_label.extend(sgd_prev_slot_label_list)
            #     self.modified_slots.extend(sgd_modified_slots)
        ...
        if 'augment' in self.data_option:
            with open(f'data/trade_slot_value_dict_augmented.pkl', 'rb') as f:
                self.slot_value_dict = pickle.load(f)
            # with open('data/trade_slot_value_dict_augmented_10_all_slot.pkl', 'rb') as f:
            #     self.slot_value_dict = pickle.load(f)
        else:
            with open(f'data/trade_slot_value_dict.pkl', 'rb') as f:
                self.slot_value_dict = pickle.load(f)
    ...
    def get_prev_ds_string(self, prev_ds_list, shuffle):
        ...
        for ii, prev_ds in enumerate(prev_ds_list):
            sequence_2 = list()
            if self.wo_none:
            ...
            print("change prev ds string permutation", ii/len(prev_ds_list))
    ...
    def reset_batch_encoding_wrt_prev_ds(self):
        ...
        prev_ds_history = self.get_prev_ds_string(self.prev_slot_label, True)

        if self.is_pretrain and self.order_shuffle:
                ...
                if cnt < 3:
                    ...
                    if np.random.uniform(0, 1) > 0.5:
                        ...
                        while(True):
                            ...
                            while True:
                                chosen = np.random.choice(list(range(0, idx-30)) + list(range(idx+30, len(self.dialog_history))))
                                if self.turn_domain[chosen] == self.turn_domain[idx]:
                                    break
                            to_be_swapped = self.dialog_history[chosen]
                            to_be_swapped_dialog_list = re.split(' \[SYS\] | \[USR\] ', to_be_swapped)[1:]
                            if len(to_be_swapped_dialog_list) >= self.max_turns:
                                print("progress of msp", idx/len(self.dialog_history))
                                break
        ...
        # prev_ds_history = self.get_prev_ds_string(self.prev_slot_label, True)
        # self.batch_encoding = self.tokenizer(text=self.dialog_history,
        #                                 text_pair=prev_ds_history,
        #                                 truncation=True,
        #                                 max_length=self.max_length,
        #                                 add_special_tokens=False,
