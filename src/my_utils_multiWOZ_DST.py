...
class Dataset(data.Dataset):
    ...
    def __init__(self, tokenizer, data_info, src_word2id, trg_word2id, sequicity, mem_word2id):
        ...
        self.sept_token_id = tokenizer.domain_cls_tokens[0]
        self.sys_start = 0
        self.usr_start = 1
    ...
    def preprocess(self, sequence, tokenizer):
        """Converts words to ids."""
        tokenized_text = tokenizer(sequence, add_special_tokens=False, return_attention_mask=True)
        sample = dict()
        sample['input_ids'] = tokenized_text['input_ids']
        sample['attention_mask'] = tokenized_text['attention_mask']
        offset_seg = torch.where(sample['input_ids'] == self.sep_token_id)[0]
        if len(offset_seg) == 2:
            offset_seg_1, offset_seg_2 = offset_seg
        ...
        elif len(offset_seg) == 1:
            sample['token_type_ids'] = torch.zeros(offset_seg + 1, dtype=torch.long)  # include segment token

        utterance_type_ids_list = list()
        usr_or_sys = self.sys_start
        if usr_or_sys == self.sys_start:
            zero_or_one = [torch.zeros, torch.ones]

        offset_utterance_type_ids = torch.where(sample['input_ids'] == self.sept_token_id)[0]

        utterance_type_ids_list.append(
            zero_or_one[0](offset_utterance_type_ids[0] + 1, dtype=torch.long))  # include [SEPT])
        prev_offset = offset_utterance_type_ids[0]
        for idx, offset in enumerate(offset_utterance_type_ids[1:], start=1):
            utt_len = offset - prev_offset
            if idx == len(offset_utterance_type_ids[1:]):
                utt_len += 1  # include [SEP]
            utterance_type_ids_list.append(zero_or_one[idx % 2](utt_len, dtype=torch.long))
            # if idx % 2 == 1:
            #     utterance_type_ids_list.append(zero_or_one[idx % 2](offset - prev_offset + 1, dtype=torch.long))
            # else:
            #     utterance_type_ids_list.append(zero_or_one[idx % 2](offset - prev_offset - 1, dtype=torch.long))
            prev_offset = offset

        utterance_type_ids = torch.cat(utterance_type_ids_list)
        sample['utterance_type_ids'] = utterance_type_ids

        # sample['sentence_order_label'] = torch.tensor(sentence_order, dtype=torch.long)
...
def add_special_token(tokenizer, seq_list):
...
def read_langs(tokenizer, file_name, gating_dict, SLOTS, dataset, lang, mem_lang, sequicity, training, max_line=None):
    ...
    with open(file_name) as f:
        ...
        for dial_dict in dials:
            ...
            for ti, turn in enumerate(dial_dict["dialogue"]):
                ...
                turn_uttr = add_special_token(turn["system_transcript"] ,turn["transcript"])
...
def prepare_data_seq(training, task="dst", sequicity=0, batch_size=100):
    ...
    if training:
        ...
        # with open('data/processed_train', 'wb') as f:
        #     pickle.dump((pair_train, train_max_len, slot_train), f)
        # with open('data/processed_dev', 'wb') as f:
        #     pickle.dump((pair_dev, dev_max_len, slot_dev), f)
        # with open('data/processed_test', 'wb') as f:
        #     pickle.dump((pair_test, train_max_len, slot_train), f)
